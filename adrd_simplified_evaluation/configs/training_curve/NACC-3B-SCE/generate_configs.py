...
config_template = """run_readable_name: "NACC-3B-SCE-{step_padded}" # will be used to name the results folder

training_steps: {step}

# passed to vllm.LLM, you can use any accepted keyword argument
LLM:
  model: "/projectnb/vkolagrp/skowshik/foundation_adrd/adrd-foundation-model/open-r1/ckpt/ckpt_access/qwen25_3B_drgrpo_gp16_nacc_inc_oversample_dedup_sce_tanh/checkpoint-{step}" # can be a path
...
benchmarks:
  ...
  run_dir: "NACC-3B-SCE" # Run folder
  max_questions: {max_questions} # Read at most this many questions from the benchmark
  benchmark_list:
{benchmark_list}
"""

config_final = """run_readable_name: "NACC-3B-SCE" # will be used to name the results folder

training_steps: 1115

# passed to vllm.LLM, you can use any accepted keyword argument
LLM:
  model: "/projectnb/vkolagrp/skowshik/foundation_adrd/adrd-foundation-model/open-r1/ckpt/ckpt_access/qwen25_3B_drgrpo_gp16_nacc_inc_oversample_dedup_sce_tanh" # can be a path
  tensor_parallel_size: 1 # Number of GPUs used. Passed to vllm.LLM.tensor_parallel_size. 
  gpu_memory_utilization: 0.9 # Fraction of gpu memory reserved for model + attention
  max_model_len: 30000
  distributed_executor_backend: 'mp'
...
for step in range(200, 1100, 200):
    ...
    filename = f"NACC-3B-SCE-{step_padded}.yml"
...
filename_final = "NACC-3B-SCE-1115.yml"
