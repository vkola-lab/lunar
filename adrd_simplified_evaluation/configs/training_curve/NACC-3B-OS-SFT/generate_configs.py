...
config_template = """run_readable_name: "NACC-3B-OS-SFT-{step_padded}" # will be used to name the results folder

training_steps: {step}

# passed to vllm.LLM, you can use any accepted keyword argument
LLM:
  model: "/projectnb/vkolagrp/skowshik/foundation_adrd/adrd-foundation-model/open-r1/ckpt/ckpt_access/qwen25_3B_sft_nacc_inc_oversample/checkpoint-{step}" # can be a path
...
config_final = """run_readable_name: "NACC-3B-OS-SFT" # will be used to name the results folder

training_steps: 1491

# passed to vllm.LLM, you can use any accepted keyword argument
LLM:
  model: "/projectnb/vkolagrp/skowshik/foundation_adrd/adrd-foundation-model/open-r1/ckpt/ckpt_access/qwen25_3B_sft_nacc_inc_oversample" # can be a path
  tensor_parallel_size: 2 # Number of GPUs used. Passed to vllm.LLM.tensor_parallel_size. 
  gpu_memory_utilization: 0.9 # Fraction of gpu memory reserved for model + attention
  max_model_len: 30000
  distributed_executor_backend: 'mp'
...
for step in range(20, 220, 20):
    ...
    filename = f"NACC-3B-OS-SFT-{step_padded}.yml"
