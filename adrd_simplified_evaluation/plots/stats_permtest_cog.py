...
DATASET_PATHS = {
    "NACC": Path(
        "/projectnb/vkolagrp/projects/adrd_foundation_model/results/NACC/test_cog"
    ),
    "ADNI": Path(
        "/projectnb/vkolagrp/projects/adrd_foundation_model/results/ADNI/test_cog"
    ),
    "NIFD": Path(
        "/projectnb/vkolagrp/projects/adrd_foundation_model/results/NIFD/test_cog"
    ),
    "PPMI": Path(
        "/projectnb/vkolagrp/projects/adrd_foundation_model/results/PPMI/test_cog"
    ),
    "BrainLat": Path(
        "/projectnb/vkolagrp/projects/adrd_foundation_model/results/brainlat/test_cog"
    ),
}
...
FIGSIZE_PR = (4.5, 4)  # precision/recall
FIGSIZE_F1 = (4.0, 2.5)  # F1-only
...
def load_answers(dir_path: Path, dataset_name: str) -> pd.DataFrame:
    """
    Load all COG parquet files from a dataset directory into a tall DataFrame.
    """
...
def build_results_df() -> pd.DataFrame:
    """
    Load all datasets, concatenate, map datasets for internal/external, and add trial index.
    """
    ...
    results_df = pd.concat(
        [nacc_res, adni_res, brainlat_res, nifd_res, ppmi_res],
        ignore_index=True,
    )
...
def _permutation_worker(task, n_perms: int, seed: int):
    ...
    obs_diff = _vectorized_metric_calc(yt, yp1, lbl_code, m_type) - _vectorized_metric_calc(
        yt, yp2, lbl_code, m_type
    )
...
def generate_latex_table(
    ...
    stats = stats.sort_values(["dataset", "class", "model"]).reset_index(drop=True)
    ...
    for row_idx, metric in enumerate(metrics):
        for col_idx, dataset in enumerate(datasets):
            ...
            for i, model in enumerate(models):
                ...
                bar_info[model]["height"] = np.array(values) + np.array(
                    errors_high
                )
            ...
            max_bar_height = max(
                (
                    np.max(bar_info[m]["height"])
                    for m in models
                    if len(bar_info[m]["height"]) > 0
                ),
                default=0,
            )
            ...
            ax.set_yticklabels(
                [f"{y:.1f}" for y in yticks_filtered],
                fontsize=FONTSIZE,
            )
            ...
            if row_idx == 0:
                title = (
                    "NACC (Internal testing)"
                    if dataset == "NACC"
                    else "All other cohorts (External testing)"
                )
...
def main() -> None:
    ...
    print("Plotting for precision and recall")
    metrics = ["precision", "recall"]
    figsize = FIGSIZE_PR
