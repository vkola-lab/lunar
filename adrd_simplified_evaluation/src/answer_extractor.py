...
class MultiAnswerExtractor:
    ...
    def extract_boxed(self, text):
        # Match only comma separated single letters, with flexible whitespace \boxed{  A , B,C} 
        # Anything else will return None. Example: \boxed{B.} -> None (there is a period)
        # If there is more than one \boxed it will pick the last one
        matches = re.findall(r"\\boxed\{\s*(\b[A-H]\b(?:\s*,\s*\b[A-H]\b)*)\s*\}", text)

        if not matches:
            return ""

        # Extract all valid single-letter options from the last match
        symbols = re.findall(r"\b[A-H]\b", matches[-1])

        if not symbols:
            return ""

        return "".join(sorted(list({s for s in symbols})))
    ...
    def extract_llm(self, ans_df):
        ...
        messages = [
            [
                {
                    "role": "user",
                    "content": prompt_templates.EXTRACT_MULTI_ANSWER_BOXED_PROMPT.format(
                        answer=row.generated_text, options=row.options
                    ),
                }
            ]
    ...
    def extract_from_file(self, file_path):
        ...
        # extract with LLM
        mask = results_df["extracted"].apply(lambda x: len(x) == 0) 

        # save invalid answers
        # results_df.loc[mask].to_json(file_path.parent / "invalid_answers.jsonl",lines=True,orient='records')

        results_df["prediction"] = results_df["extracted"].copy()

        predictions = self.extract_llm(results_df.loc[mask, ["generated_text", "options"]])

        results_df.loc[mask, "prediction"] = predictions
