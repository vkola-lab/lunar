        ...
        if metrics_path.is_file(): 
            ...
            for attempt,group in ans_df.groupby('attempt'):
                ...
                if any([len(label) != 1 for label in labels]):
                    ...
                    metrics["precision_weighted"] = precision_score( y_true, y_pred, average="weighted", zero_division=0,labels=labels)
                    metrics["f1_weighted"] = f1_score( y_true, y_pred, average="weighted", zero_division=0,labels=labels)

                    # same thing as balanced accuracy
